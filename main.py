...
def run_slam(src_dir, log_dir, idx, split):
    ...
    p = np.zeros((3,n))
    slam.init_particles(n, p, w)
    best_particles = np.zeros((3,T))

    xyth = []
    for p in slam.lidar:
        xyth.append([p['xyth'][0], p['xyth'][1],p['xyth'][2]])
    xyth = np.array(xyth).T
    xy_od = slam.map.grid_cell_from_xy(xyth[0],xyth[1])

    for i in tqdm.tqdm(range(1,T)):  
        slam.observation_step(t=i)
        slam.dynamics_step(t=i)
        best_particles[:,i] = slam.best_particle
        slam.resample_particles()

    best_particle_idx = slam.map.grid_cell_from_xy(best_particles[0],best_particles[1])

    plot_map_path(log_dir,split,idx,'log_odd',slam.map.log_odds,best_particle_idx,xy_od)
    plot_map_path(log_dir,split,idx,'cells',slam.map.cells,best_particle_idx,xy_od)
        
def plot_map_path(log_dir,split,idx,map_type,map,best_particles_idx,xy_od):
    plt.clf()
    plt.gcf().set_size_inches(10, 8)
    plt.imshow(map, cmap='binary', interpolation='nearest')
    plt.plot(xy_od[1],xy_od[0],color='r', label='Odometry Trajectory')
    plt.plot(best_particles_idx[1], best_particles_idx[0], color='b', label='Particle Filter Trajectory')
    
    plt.legend()
    plt.savefig(os.path.join(log_dir, '%s_map_%s_%02d.jpg'%(map_type,split, idx)), dpi=300)
